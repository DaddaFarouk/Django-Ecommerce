...
def product_detail(request, category_slug, product_slug):
    ...
    # Check if the user has purchaced this product
...
def submit_review(request, product_id):
    url = request.META.get('HTTP_REFERER')  # store the current place url
    if request.method == 'POST':
        try:
            reviews = ReviewRating.objects.get(user__id=request.user.id, product__id=product_id)
            form    = ReviewForm(request.POST, instance=reviews) # if this user has already a review then we update it
            form.save()
            ...
            if form.is_valid():
                ...
                data.ip     = request.META.get('REMOTE_ADDR') # store the ip adress
